...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", function (event) {
            ...
            if (username === adminUsername && password === adminPassword) {
                ...
                // Αν έχει απαντήσει το ερωτηματολόγιο, πήγαινε στο user_results
                if (user.hasAnswered) {
                    setTimeout(() => {
                        window.location.href = "user_results.html";
                    }, 100);
                } else {
                    // Αν όχι, μείνε στο instructions
                    setTimeout(() => {
    ...
    function loadUsers() {
        ...
        if (userListContainer) {
            ...
            if (users.length === 0) {
                userListContainer.innerHTML = "<tr><td colspan='5'>Δεν υπάρχουν χρήστες προς έγκριση.</td></tr>";
            }
    ...
    function exportToCSV() {
        ...
        let headers = "ΧΡΗΣΤΗΣ,ΥΠΗΡΕΣΙΑ,ΟΡΓΑΝΙΣΜΟΣ,ΑΝΘΡΩΠΟΚΕΝΤΡΙΚΟΤΗΤΑ,ΚΛΙΝΙΚΗ ΑΠΟΤΕΛΕΣΜΑΤΙΚΟΤΗΤΑ,ΑΣΦΑΛΕΙΑ - ΑΠΟΡΡΗΤΟ,ΣΥΝΟΛΟ";
        for (let i = 1; i <= 16; i++) {
            headers += `,ΕΡΩΤΗΣΗ ${i}`;
        }
        ...
        data.forEach(user => {
            ...
            if (totalScore > 0) {
                let row = `${user.username},${user.serviceName},${user.organizationName},${user.score1},${user.score2},${user.score3},${totalScore}`;
    ...
    function exportToTXT() {
        ...
        let textContent = "ΧΡΗΣΤΗΣ | ΥΠΗΡΕΣΙΑ | ΟΡΓΑΝΙΣΜΟΣ | ΑΝΘΡΩΠΟΚΕΝΤΡΙΚΟΤΗΤΑ | ΚΛΙΝΙΚΗ ΑΠΟΤΕΛΕΣΜΑΤΙΚΟΤΗΤΑ | ΑΣΦΑΛΕΙΑ - ΑΠΟΡΡΗΤΟ | ΣΥΝΟΛΟ";
        for (let i = 1; i <= 16; i++) {
        ...
        data.forEach(user => {
            const totalScore = (parseInt(user.score1) || 0) + (parseInt(user.score2) || 0) + (parseInt(user.score3) || 0);
            if (totalScore > 0) {
                let rowText = `${user.username}| ${user.serviceName} | ${user.organizationName} | ${user.score1} | ${user.score2} | ${user.score3} | ${totalScore}`;
                const answerValues = user.answerValues || [];
